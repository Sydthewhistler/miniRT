...
/*   Updated: 2025/11/26 13:46:32 by cprot            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	parse_ambient(char **line, t_environment **env)
{
	...
	if (!(*env)->ambient)
		exit_with_error("Error: Memory allocation failed for ambient\n", env,
			NULL, NULL);
...
void	parse_camera(char **line, t_environment **env, t_object **obj)
{
	...
	if (!(*env)->camera)
		exit_with_error("Error: Memory allocation failed for camera\n", env,
			obj, NULL);
...
void	parse_light(char **line, t_environment **env)
{
	...
	if (!(*env)->light)
		exit_with_error("Error: Memory allocation failed for light\n", env,
			NULL, NULL);
